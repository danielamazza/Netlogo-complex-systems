...
to-report initial-condition report
  ifelse-value random-start?
    ...
    [n-values world-width [? = max-pxcor - 2 - floor (world-width / 2)]]
end
...
to display-history
  ...
  [
    ...
    foreach n-values (min list n world-height) [min-pycor + ?]
    [display-row (max-pycor - ?) (item (? + scroll-offset) history)]
  ]
  [
    let n (length history) - 1 
    ask patches [set pcolor grey + 2]
    foreach n-values (min list n world-height) [min-pycor + ?]
    [display-row (max-pycor - ?) (item (?) history)]
  ]
...
to setup-table
  ...
  foreach n-values 8 [?]
    [ask patch (3 + 4 * ?) 41 [sprout-cells 1 [hide-turtle]]
